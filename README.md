...
| 插件 | 类型 | 说明 |
| --- | --- | --- |
| [dsh-plugin-model-autofill](plugins/dsh-plugin-model-autofill) | host | 在 `llm-pi-ai` 设置命名空间中自动补全模型显示名 / 上下文窗口 / 最大输出 token（来自 pi-ai 内置目录） |
| [dsh-plugin-workspace-artifacts](plugins/dsh-plugin-workspace-artifacts) | host + client | 「产物」视图：在会话视图中浏览当前工作区文件树、查看文件内容（代码高亮 / Markdown 渲染） |
...
> 声明了 `prepare` 脚本时），在 profile 的 `pnpm-workspace.yaml` 中放行对应包名即可。
